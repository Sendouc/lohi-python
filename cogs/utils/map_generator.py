...
# Credits to Lean https://github.com/Leanny/SplatoonAlgorithm/blob/master/map_generator/map_generator.py
    ...
    for round in range(len(games)):
        ...
        for game in range(games[round]):
            ...
            if popularity is None:
                ...
                new_map = get_map_poopular(
                    map_pool, mode_list[mode_index], popularity, map_history
                )
...
def get_map_poopular(
    map_pool: dict, mode: str, popularity: dict, map_history: list
) -> str:
    """
    ...
    popularity_map_pool = {
        mapname: votes
        for mapname, votes in popularity[mode].items()
        if mapname in map_pool[mode]
    }
